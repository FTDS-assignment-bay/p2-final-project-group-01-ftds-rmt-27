...
# Load the models
banten_model = joblib.load('banten.joblib')
jakarta_model = joblib.load('jakarta.joblib')
jawa_barat_model = joblib.load('jawa_barat.joblib')
jawa_tengah_model = joblib.load('jawa_tengah.joblib')
jawa_timur_model = joblib.load('jawa_timur.joblib')
yogyakarta_model = joblib.load('yogyakarta.joblib')

# Load the datasets
jakarta_df = pd.read_csv('jakarta_premium.csv')
banten_df = pd.read_csv('banten_premium.csv')
jawa_barat_df = pd.read_csv('jawa_barat_premium.csv')
jawa_tengah_df = pd.read_csv('jawa_tengah_premium.csv')
jawa_timur_df = pd.read_csv('jawa_timur_premium.csv')
yogyakarta_df = pd.read_csv('yogyakarta_premium.csv')
...
page = st.sidebar.selectbox(label='Choose Page', options=['Home','Predict', 'News'])
...
if page == 'Home':
    ...
    with st.form("Input the province and length of prediction"):
        ...
        n_month = st.slider('Months of prediction since 2023-03-01:', 1, 6)
        ...
        if sub:
            ...
            # Display the budget
            st.header("Monthly Budget Reccomendation")
            monthly_budget = (province_forecast_poly.tail(30)[1].mean()) * 30
            st.write(f"The recommended monthly budget based on the forecast is approximately Rp{monthly_budget:.2f}")
